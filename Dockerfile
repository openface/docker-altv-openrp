...
RUN wget -nv --show-progress --progress=bar:force:noscroll -O /tmp/altv-open-roleplay.zip https://github.com/team-stuyk-alt-v/altV-Open-Roleplay/archive/master.zip && \
	...
	rm /tmp/altv-open-roleplay.zip && rm -rf /tmp/altV-Open-Roleplay-master && \
	npm install
...
ADD database.json.template ./resources/orp/server/configuration/database.json.template
...
ADD terms-and-conditions.json ./resources/orp/terms-and-conditions.json
...
ADD server.cfg.template server.cfg.template
RUN envsubst < ./server.cfg.template > ./server.cfg

# Cleanup
RUN apt-get purge -y wget unzip curl && apt-get clean && rm server.cfg.template && rm ./resources/orp/server/configuration/database.json.template
